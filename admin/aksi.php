 ...
 {  
 	...
 	if($set) 
 		...
 		$nm_galeri=$_POST['nm_gal'];   
 	 	$ket=$_POST['ket'];   
 	 	$tgl=date('d n Y');   
 	 	$sql=mysqli_query($koneksi,"insert into galeri(nm_galeri,ket,tgl_galeri,gambar)values('$nm_galeri','$ket' ,'$tgl','$nama_file')");   
 		...
 		//tentukan variabel file yg diupload dan tipe file 
 		$tipe_file   = $_FILES['gam_baru']['type']; $lokasi_file = $_FILES['gam_baru']['tmp_name']; $nama_file  = $_FILES['gam_baru']['name']; $save_file =move_uploaded_file($lokasi_file,"galeri/$nama_file");    
 		if(empty($lokasi_file)) 
 		{  
 		 isset($set); 
 		} 
